...
public:
   ...
   static void GenerateRandomLatticesAndCatalogByReductionStepCount(const size_t n);
   static std::pair<S6, S6> GenerateLatticeTypeExamplesNearDeloneTypes(const size_t n);
   static S6 DoRandomReflection(const S6& s);


   static void MultiGeneratePair_Reduced_Other_ReducesFarAway(const size_t targetReducedUnreducedPairCount);
...
private:
   static bool IsGoodLatticeTypeForPerturbation(const size_t n);
   static std::pair<S6, S6> OneLatticeType(const MatS6& m);
   static std::pair<S6, S6> OneLatticeAttempt(const MatS6& m);
   static std::pair<S6, S6> GeneratePair_Reduced_Other_ReducesFarAway(const size_t numberOfZeros);
   static S6 DoReflection(const S6& s, const size_t n);
   ...
   static void StoreOnePointIfValid(StoreResults<size_t, std::string >& store, const S6& s6in);
};
