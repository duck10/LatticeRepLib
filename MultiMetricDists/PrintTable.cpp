...
void PrintTable::InsertRowSeparators( void ) {
   ...
   std::string sep;
   for ( int i = 0; i<(int)(lineLength) - 1; ++i ) sep += m_rowSeparator;
   sep += "\n";
   for (int i = (int)(m_table.size( )); i >= 0; i -= (int)(lineLength)) {
      if ( i > 0 && m_table[i - 1] == '\n' ) {
         //--i;
      }
...
void PrintTable::NumberColumns( void ) {
   ...
   for ( size_t i = 0; i < m_colCount; ++i ) {
      const std::string num = LRL_ToString( i );
...
void PrintTable::NumberRowWithoutRowSeparators( ) {
   ...
   for ( size_t i = 0; i < m_rowCount; ++i ) {
      vs.push_back( LRL_ToString( m_rowCount - i ) + "  " );
   }
   ...
   for ( size_t i = m_table.size( ) - lineLength; i >= 0; i -= lineLength ) {
