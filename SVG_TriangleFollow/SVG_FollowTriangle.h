...
public:
   ...
   std::string DrawOneY_AxisLabel(const double y, const double yscale, const double value) {
      ...
      return(
         ...
         + " x=\"" + LRL_ToString(-SVG_DistancePlotConstants::globalY_AxisTicMarkLength + 5) + "\""
         + " y=\"" + LRL_ToString(-y + 10) + "\" >"
         + returnString + "</text>");
   }
   ...
   std::vector<std::string> FormatAllPointsAsPolyline(const AxisLimits& alX, const AxisLimits& alY, const ProgressData<double>& d, const double minValue, const double range, const std::string& color)
      /*-------------------------------------------------------------------------------------*/
   {
      /* LOCAL */ class CalculateOneGraphScaling {
      public:
         CalculateOneGraphScaling(const AxisLimits& alX, const AxisLimits& alY, const int xmax, const int borderWidth, const int plotHeight, const int plotWidth)
            : m_borderWidth(borderWidth)
            , m_plotHeight(plotHeight)
            , m_plotWidth(plotWidth)
         {
            m_dataLineXDelta = ActualLineXLength(alX, alY) / double(xmax - 1);
            m_xStartPos = m_dataLineXDelta * (alX.m_minDataValue - alX.m_lowerLimit);

            m_yStartPos = CalculateYStartPos(alY);
            m_yscale = (0.9*m_plotHeight - m_borderWidth) / (alY.m_upperLimit - alY.m_lowerLimit);
         }

         double ActualLineXLength(const AxisLimits& alX, const AxisLimits& alY) {
            return (0.9*m_plotWidth - m_borderWidth)*(alX.m_maxDataValue - alX.m_minDataValue) / (alX.m_upperLimit - alX.m_lowerLimit);
         }

         const double CalculateYStartPos(const AxisLimits& alY) {
      ...
      CalculateOneGraphScaling ts(alX, alY, xmax, m_borderWidth, m_plotHeight, m_plotWidth);

      std::string errorString;
      std::vector<std::string> errorSegments;

      bool previousPointIsError = false;
      std::string svgGlitches;

      for (int i = 0; i<xmax; ++i) {
         const double xpos = m_borderWidth + ts.m_xStartPos + (double)(i)*ts.m_dataLineXDelta;
         const double startPoint = ts.CalculateYStartPos(alY);
         const double ypos = startPoint + ts.m_yscale * (d[i] - minValue);

         svgGlitches += DrawGlitch(setOfGlitches, i, xpos, ypos);
